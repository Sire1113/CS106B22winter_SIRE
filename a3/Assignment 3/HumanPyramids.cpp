 ...
 */
double weightOnBackOfHelper(int row, int col, int pyramidHeight,Map<Set<int>,double>& table);

double weightOnBackOf(int row, int col, int pyramidHeight) {
    /* TODO: Delete the next few lines and implement this function. */
    //cout << "col: "<< col << " row: " << row << " pyramid: " << pyramidHeight <<endl;
    Map<Set<int>,double> table;
    return weightOnBackOfHelper(row,col,pyramidHeight, table);
}


double weightOnBackOfHelper(int row, int col, int pyramidHeight, Map<Set<int>,double>& table){
    if (row < 0 || col < 0 || row > pyramidHeight -1 || col > row + 1){
        error("input error");
    }
    else {
        double weight = 80;
        // Map<Set<int>,double> tab;
        Set<int> params = {row, col};
        if (row == 0){
            return 0;
        }
        else if (table.containsKey(params)){
            return table[params];
        }
        else {
            if (col == 0){
                double result = weight + weightOnBackOfHelper(row-1, col, pyramidHeight, table)/2;
                table[params] = result;
                return result;
            }
            else if(col == row){
                double result = weight + weightOnBackOfHelper(row-1, col-1, pyramidHeight, table)/2;
                table[params] = result;
                return result;
            }
            else{
                double result = weight * 2 + weightOnBackOfHelper(row-1, col, pyramidHeight, table)/2 +
                                weightOnBackOfHelper(row-1, col-1, pyramidHeight, table)/2;
                table[params] = result;
                return result;
            }
        }
