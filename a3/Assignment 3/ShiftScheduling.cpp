 ...
 */
Set<Shift> highestValueScheduleForHelper(const Set<Shift>& shifts, int maxHours,Set<Shift>& chosen){
    Set<Shift> shifts_copy = shifts,c,c1;
    if(maxHours < 0){
        error("input error");
    }
    else if (maxHours ==0 || shifts.isEmpty()){
        //base case
        return c + chosen;
    }
    else {
        Shift s = shifts_copy.first();
        int max_value=0;
        shifts_copy.remove(s);
        // without s
        c = highestValueScheduleForHelper(shifts_copy,maxHours,chosen);
        int temp_value=0;
        for (Shift each_shift:c){
            max_value+=valueOf(each_shift);
        }
        //with s
        bool flag = true;
        for (Shift c1:chosen){
            if (overlapsWith(c1,s)){
                flag = false;
            }
        }
        if (maxHours - lengthOf(s) < 0){
            flag = false;
        }
        if(flag){
            chosen.add(s);
            c1 = highestValueScheduleForHelper(shifts_copy,maxHours - lengthOf(s),chosen);

            for (Shift each_shift:c1){
                temp_value+=valueOf(each_shift);
            }
        }
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    Set<Shift> chosen;
    return highestValueScheduleForHelper(shifts,maxHours,chosen);
}
