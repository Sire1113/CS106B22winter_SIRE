...
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    ...
    max_size = hashFn.numSlots();
    elems = new Slot[max_size]();
    hash_func = hashFn;
    num_elems = 0;
...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    if (max_size==num_elems)
        return false;
    if (contains(elem)){
        return false;
    }
    int index = hash_func(elem);
    int iter = 0;
    if (elems[index].distance==EMPTY_SLOT){
        elems[index].value = elem;
        elems[index].distance = 0;
        iter++;
        index++;
        num_elems++;
        return true;
        ...
        auto to_swap = elem;
        while (!(elems[index].distance==EMPTY_SLOT)){
            if (iter > elems[index].distance){
                swap(iter,elems[index].distance);
                swap(elems[index].value,to_swap);
            }
            iter++;
            if (index==max_size-1){
                index=0;
        ...
        elems[index].distance = iter;
        elems[index].value = to_swap;
    }
    num_elems++;
    return true;
}
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    while (!(elems[index].distance==EMPTY_SLOT)){
        if (find){
            if (elems[index].distance == 0){
                break;
            }
            else{
                swap(elems[index],elems[last]);
                elems[last].distance--;
            }
        }
        else{
...
void RobinHoodHashTable::printDebugInfo() const {
    ...
    for (int i=0;i<max_size;i++){
        std::cout<<"index: "<<i<<" value: "<<elems[i].value<<" distance: "<<elems[i].distance<<endl;
    }
}
