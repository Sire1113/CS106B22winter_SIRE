...
Nucleotide* toStrand(const string& str) {
    ...
    Nucleotide* new_dna = new Nucleotide;
    new_dna->prev = nullptr;
    new_dna->next = nullptr;
    auto p = new_dna;
    for (char val:str){
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    if (target == nullptr)
        return dna;
    auto search = target;
    Nucleotide* res = nullptr;
    while (dna!=nullptr){
        if (dna->value == target->value){
            res = dna;
            while (search!=nullptr){
                if (dna==nullptr || dna->value != search->value)
                    break;
                search = search->next;
                dna = dna->next;
            }
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    // if(dna == nullptr)
    //     return true;
    auto cut1 = findFirst(dna,target);
    auto path = target;
    Nucleotide* cut2 = cut1;
    if (cut1 == nullptr){
        return false;
    }
    if (cut1->prev ==nullptr){
        while (path != nullptr){
            path = path->next;
            auto trash = cut1;
            cut1 = cut1->next;
