...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    if (logicalSize == allocatedSize -1){
        allocatedSize *=2;
        DataPoint* new_array = new DataPoint[allocatedSize];
        auto trash = elems;
    ...
    while (data.weight < elems[father_index].weight && index > 1){
        swap(elems[father_index],elems[index]);
        index = father_index;
        father_index /=2;
    }
}
...
DataPoint HeapPQueue::dequeue() {
    ...
    int father = 1;
    int child = father * 2;
    while(father < logicalSize && child <= logicalSize){
        if (child+1 <= logicalSize  && elems[child+1].weight < elems[child].weight){
            child +=1;
        }
        if (elems[father].weight > elems[child].weight){
            swap(elems[father],elems[child]);
            father = child;
            child = father * 2;
        }
        else{
...
void HeapPQueue::printDebugInfo() {
    ...
    for (int i =1;i <= logicalSize;i++){
        cout<<"index: "<<i<<" name: "<<elems[i].name<<" weight:"<<elems[i].weight<<endl;
    }
}
