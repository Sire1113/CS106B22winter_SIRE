...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    Map<string, Set<string>> possible = possibleLinks;
    string first = possible.firstKey();
    Set<string> first_match = possible[first];
    possible.remove(first);
    for (string sel:first_match){
        if (possible.containsKey(sel)){
            Set<string> to_add = possible[sel];
            possible.remove(sel);
            Pair c1(first,sel);
            if (hasPerfectMatching(possible,matching)){
                matching.add(c1);
                return true;
            }
            possible.put(sel,to_add);
        }
    }
    return false;
}

int get_weight(const Set<Pair>& pairs, const Map<string, Map<string, int>>& possibleLinks){
    int total_weight = 0;
    for (Pair p:pairs){
        string p1 = p.first();
        string p2 = p.second();
        int weight = possibleLinks[p1][p2];
        total_weight += weight;
    }
    return total_weight;
}
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    /* TODO: Delete this comment and these remaining lines, then implement this function. */
    Set<Pair> selections,s1,s2,s_max;
    if (possibleLinks.isEmpty()){
        return selections;
    }
    Map<string, Map<string, int>> possibles = possibleLinks;
    int max_weight = 0;
    Pair c2;
    string first = possibles.firstKey();
    Map<string, int> first_match = possibles[first];
    possibles.remove(first);
    // without first
    s2 = maximumWeightMatching(possibles);
    int weight_no_first = get_weight(s2,possibleLinks);
    if (weight_no_first > max_weight){
        max_weight = weight_no_first;
        s_max = s2;
    }
    //with first
    for (string sel:first_match){
        if (possibles.containsKey(sel)){
            int this_weight = first_match[sel];
            Map<string, int> to_add = possibles[sel];
            possibles.remove(sel);
            s1 = maximumWeightMatching(possibles);
            int weight = this_weight + get_weight(s1,possibleLinks);
            possibles.put(sel,to_add);
    ...
    possibles.put(first,first_match);
...
PROVIDED_TEST("hasPerfectMatching works on a line of six people.") {
    ...
    do {
        ...
        Set<Pair> matching;
        bool flag1 = hasPerfectMatching(links, matching);
        EXPECT(flag1);
        bool flag2 =isPerfectMatching(links, matching);
        EXPECT(flag2);
