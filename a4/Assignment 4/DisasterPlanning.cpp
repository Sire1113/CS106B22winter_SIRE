...
bool canBeMadeDisasterReadyHelper(const Map<string, Set<string>>& roadNetwork,
                            int numCities,
                                  Set<string>& supplyLocations,Set<string>& uncover){
    if (numCities < 0){
        error("input error");
    }
    if (uncover.isEmpty()){
    ...
    string first = uncover.first();
    Set<string> first_match = roadNetwork[first],diff;
    // with first
    uncover.remove(first);
    supplyLocations.add(first);
    Set<string> temp = roadNetwork[first];
    diff = uncover - first_match;
    if(canBeMadeDisasterReadyHelper(roadNetwork, numCities - 1, supplyLocations,diff)){
        return true;
    }
    supplyLocations.remove(first);
    //without first
    for (string city:first_match){
        bool flag = false;
        if (uncover.contains(city)){
            flag = true;
        }
        supplyLocations.add(city);
        uncover.remove(city);
        temp = roadNetwork[city] ;
        diff = uncover - temp;
        if (canBeMadeDisasterReadyHelper(roadNetwork,numCities -1, supplyLocations,diff)){
            return true;
        }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    return canBeMadeDisasterReadyHelper(roadNetwork,numCities,supplyLocations,uncoveredCity);
}
