...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if(str.length() >= kGramLength){
        for(int i = 0;i < str.length() - kGramLength + 1;i += 1){
            string s = str.substr(i,kGramLength);
            kGrams[s] += 1;
        }
    }
    else{
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for(int i = 0;i < keys.size();i++){
        sum_sqrt += pow(input[keys[i]],2);
    }
    if(sum_sqrt == 0){
        error("the input is all the zero");
    }
    for(int i = 0;i < keys.size();i++){
        norm[keys[i]] = input[keys[i]] / sqrt(sum_sqrt);
    }
    return norm;
}
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    int pq_cls = pq.size();
    for(int i = 0;i < pq_cls;i++)
        if(i < pq_cls - numToKeep){
            pq.dequeue();
        }
        else {
            topKGrams[pq.peek()] = source[pq.peek()];
            pq.dequeue();
        }
    return topKGrams;
}
