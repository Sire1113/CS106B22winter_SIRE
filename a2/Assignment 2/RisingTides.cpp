...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> map(terrain.numRows(),terrain.numCols());
    map.fill(false);
    Queue<GridLocation> candidate;
    GridLocation loc,loc_;
    if (sources.isEmpty()){
        return map;
    }
    for (GridLocation loc1:sources){
        if (terrain.inBounds(loc1)){
            if(terrain.get(loc1) <= height){
                map[loc1] = true;
                candidate.enqueue(loc1);
            }
        }
    }
    Vector<int> m= {-1,1};
    while (!candidate.isEmpty()){
        loc = candidate.dequeue();
        loc_ = {loc.row + 1, loc.col};
        if (terrain.inBounds(loc_)){
            if (terrain.get(loc_) <= height && !map.get(loc_)){
                map[loc_] = true;
                candidate.enqueue(loc_);
            }
        }
        loc_ = {loc.row - 1, loc.col};
        if (terrain.inBounds(loc_)){
