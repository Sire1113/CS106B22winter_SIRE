...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    if (sequences.size() == 0){
        ...
        auto left = sequences[0];
        auto right = sequences[1];
        if (left.isEmpty()){
            return right;
        }
        else {
        ...
        int  q = 0;
        bool left_bottom = false,right_bottom = false;
        for (int p = 0; p <= left.size(); p++){
            while (q < right.size() ) {
                if (left_bottom){
                    ...
                    if (right[q].weight < left[p].weight){
                        result+= right[q];
                        q++;
                    }
                    else{
        ...
        auto left = sequences.subList(0, sequences.size()/2);
        auto right = sequences.subList(sequences.size()/2);
        auto merge_left = combine(left);
        auto merge_right = combine(right);
        Vector<Vector<DataPoint>> merge = {merge_left,merge_right};
        auto result = combine(merge);
        return result;
    }
}
