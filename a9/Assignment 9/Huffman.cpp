...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    auto pq = PriorityQueue<EncodingTreeNode*>();
    Map<char,int> weight;
    Set<EncodingTreeNode*> forest;
    for (auto s:str){
        weight[s] ++;
    }
    if (weight.size() < 2){
    ...
    for (char s:weight){
        ...
        tree->one = nullptr;
        tree->zero = nullptr;
        pq.enqueue(tree,weight[s]);
    }
    // main loop
    while (pq.size() != 1){
        int weight1 = pq.peekPriority();
        auto tree1 = pq.dequeue();
        int weight2 = pq.peekPriority();
        auto tree2 = pq.dequeue();
        int weight_new = weight1 + weight2;
        EncodingTreeNode* tree = new EncodingTreeNode;
        tree->zero = tree1;
        tree->one = tree2;
        pq.enqueue(tree,weight_new);
    }
 ...
 */
Vector<Bit> encodeTextHelper(const char& s, EncodingTreeNode* tree,Vector<Bit>& bits){
    Vector<Bit> res;
    ...
    auto subtree0 = tree->zero;
    bits.add(0);
    auto bit = encodeTextHelper(s,subtree0,bits);

    if (!bit.isEmpty()){
        return bit;
    }
    bits.remove(bits.size()-1);
    auto subtree1 = tree->one;
    bits.add(1);
    bit = encodeTextHelper(s,subtree1,bits);
    if (!bit.isEmpty()){
        return bit;
    }
    bits.remove(bits.size()-1);
...
Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    ...
    for (auto s:str){
        Vector<Bit> bits;
        auto bit = encodeTextHelper(s,tree,bits);
        if (!bit.isEmpty()){
            for (auto b:bit){
                res.enqueue(b);
            }
...
EncodingTreeNode* decodeTree(Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    if (bit == 0){
        auto leaf = leaves.dequeue();
        EncodingTreeNode* subtree = new EncodingTreeNode{leaf,nullptr,nullptr};
        return subtree;
    }
    else {
        EncodingTreeNode* subtree = new EncodingTreeNode;
        EncodingTreeNode* subtree0 = decodeTree(bits,leaves);
        EncodingTreeNode* subtree1 = decodeTree(bits,leaves);
        subtree->zero = subtree0;
        subtree->one = subtree1;
        return subtree;
    }
...
HuffmanResult compress(const string& text) {
    /* TODO: Delete this comment and the next few lines, then implement this. */
    auto huf_tree = huffmanTreeFor(text);
    auto text_bits = encodeText(text,huf_tree);
    Queue<Bit> tree_bits;
    Queue<char> tree_leaves;
    encodeTree(huf_tree,tree_bits,tree_leaves);
    HuffmanResult result{tree_bits,tree_leaves,text_bits};
    deleteTree(huf_tree);
    return result;
}
...
string decompress(HuffmanResult& file) {
    /* TODO: Delete this comment and the next few lines, then implement this. */
    auto text_bits = file.messageBits;
    auto  tree_bits = file.treeBits;
    auto tree_leaves = file.treeLeaves;
    auto huf_tree = decodeTree(tree_bits,tree_leaves);
    string text = decodeText(text_bits,huf_tree);
    deleteTree(huf_tree);
    return text;
}
